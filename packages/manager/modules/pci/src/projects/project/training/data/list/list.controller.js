export default class PciTrainingJobsListController {
  /* @ngInject */
  constructor($state, PciProjectStorageContainersService, CucCloudMessage, CucRegionService) {
    this.$state = $state;
  ...
  getContainerId(region, name) {
    var filtered = this.allContainers.filter((container) => {
