export default class PciTrainingJobsSubmitController {
  ...
  computePricePerMinute() {
    var price = this.resources.cpu * this.resources.mem * 0.1
  ...
  onRegionSubmit() {
    console.log('onRegionSubmit...' + JSON.stringify(this.job));
  }

  onStepperFinish() {
    const jobsSpec = this.computeJobSpec();
    console.log('onStepperFinish...' + JSON.stringify(jobsSpec));
    this.submitJob(jobsSpec);
  }

  onUserSubmit() {
    console.log('onUserSubmit...' + JSON.stringify(this.job));
  }
